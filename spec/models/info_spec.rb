...
describe Info do
  ...
  describe "static dates statistics" do
    ...
    before :each do
      ...
      infos = [today, start_of_this_week, end_of_last_week, start_of_last_week]
  ...
  describe "dynamic dates statistics"  do
    before :each do
      last_month = Info.new(:created_at => Time.today.last_month) #forrige måned
      start_of_last_month = Info.new(:created_at => Time.today.beginning_of_month.yesterday.beginning_of_month) #forrige måned
      start_of_this_month = Info.new(:created_at => Time.today.beginning_of_month) #denne måned
     infos = [last_month, start_of_this_month, start_of_last_month]

      Info.should_receive(:find).with(:all, :conditions => "created_at >= #{Time.today.last_month.beginning_of_month.to_date}").and_return(infos)
    end

    it "should have number of searches this month" do
      Info.sok_statistikk[:denne_maaned].should == 1
    end
